...
def generate_year_pages(year: int) -> None:
    ...
    body += "</div></div></section>"
    body += '<section class="section"><div class="container"><div class="section-title"><div><h2>Calendário mês a mês</h2><p>Vermelho indica feriado/folga considerada na contagem padrão; amarelo indica data especial.</p></div></div><div class="month-grid">'
...
def generate_month_pages(year: int) -> None:
    for month in range(1, 13):
        ...
        body += f'<section class="section"><div class="container"><section class="month month--large"><h2>{month_name.capitalize()} {year}</h2>{render_month(year, month)}</section></div></section>'
        if month_holidays:
